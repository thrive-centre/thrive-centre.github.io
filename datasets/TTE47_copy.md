...
<div class="navbar">
  <!-- <button class="nav-btn secondary" data-cat="all" onclick="showCat('all')">Show all</button> -->
...
<script>
  function showCat(cat){
    ...
    // smooth scroll to the visible section (skip when showing all)
    if(cat!=='all'){
      const id = (cat==='mmode') ? 'm-mode' : (cat==='subsup' ? 'subcostal' : cat);
      const target = document.getElementById(id);
      if(target) target.scrollIntoView({behavior:'smooth', block:'start'});
    }
  }
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    const initial = location.hash ? location.hash.replace('#','') : 'all';
    const valid = ['all','apical','plax','psax','mmode','subsup','doppler'];
    showCat(valid.includes(initial) ? initial : 'all');
  });

  window.addEventListener('hashchange', ()=>{
    const cat = location.hash ? location.hash.replace('#','') : 'all';
    const valid = ['all','apical','plax','psax','mmode','subsup','doppler'];
    showCat(valid.includes(cat) ? cat : 'all');
  });
</script>
 -->
